...
main_obj_name, pose_id = prs.pars_2_args(sys.argv)
...
def generate_random_poses(num_poses: int, object_id: str,
    ...
    positions, rotations = generate_non_uniform_poses(num_frustums, num_poses, max_distance, min_distance, fov,
                                                      camera_position)
...
def generate_points_in_frustum(num_points: int, max_dist: float, min_dist: float, fov: int,
...
def save_vals_to_file(output_directory: str, output_file: str, positions: np.ndarray, orientations: np.ndarray,
                      object_id: str):
    ...
    with open(os.path.join(output_directory, output_file), 'w') as f:
        for i, (orientation, position) in enumerate(zip(orientations, positions)):
            f.write(
                f"{i:05d},{object_id},{orientation[0]:.6f},{orientation[1]:.6f},{orientation[2]:.6f},{orientation[3]:.6f},{position[0]:.6f},{position[1]:.6f},{position[2]:.6f}\n")
